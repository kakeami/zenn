...
![時刻×スコアヒートマップ](/images/2026-03-iphone-github-lifelog/heatmap_hour_score.png)
*各時間帯でスコアがどう分布しているか*

22〜23時台はスコア5が200件超と圧倒的に多く、記録のボリュームゾーンであると同時に好調な時間帯であることがわかります。一方、5〜6時台はスコア2が31〜66件と目立ちます。早朝に記録している時は低調な傾向があるようです。また、スコア1は全時間帯で数件以下と非常に少なく、本当に辛い瞬間にしか付けていないことが改めて確認できます。

### 曜日 × スコア分布

![曜日×スコアヒートマップ](/images/2026-03-iphone-github-lifelog/heatmap_weekday_score.png)
*各曜日でスコアがどう分布しているか*

どの曜日もスコア4〜5に記録が集中しています。注目すべきは金曜日のスコア1が11件と突出している点です。週末に向けた疲労の蓄積が影響しているのかもしれません。

### 月 × スコア分布

![月×スコアヒートマップ](/images/2026-03-iphone-github-lifelog/heatmap_month_score.png)
*月ごとのスコア分布の変遷*

月を追うごとに全体の傾向が見えてきます。

- **2025-11**: スコア4が322件と最多で、安定した月だったことがわかります
- **2025-12**: スコア1が7件、スコア2が71件と、低スコアの割合がやや増えています。先ほどの推移グラフで見た12月の落ち込みと一致します
- **2026-01**: スコア5が462件と全期間で最多。年明けに好調だった様子が数値に表れています
